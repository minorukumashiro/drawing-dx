...
exports.extractFields = onRequest({
  ...
  try {
    ...
    const itemProps = {
      ...
      size:     { type: 'string', description: 'サイズ。内径ホーニング加工の丸物なら「仕上げ内径-全長」形式（例 37-777, 65-163, 115-487）。φ記号は付けず数字だけにする。素材外径や下穴径ではなく、仕上げ後の内径を使うこと。それ以外は図面の主要寸法表記（例 φ80×t12）。無ければ空文字。' },
    ...
    const system = [
      ...
      '・ホーニングは内径加工なので、丸物パイプ/シリンダは内径と全長を優先し size は「仕上げ内径-全長」形式にする（例 内径φ65・全長163 → 65-163）。φ記号は付けず数字だけ。素材外径や下穴径ではなく、仕上げ後の内径を使う。',
      ...
      '',
      'まず、画像が「技術図面」か「表形式の依頼書」かを見分けて、読み方を切り替える。',
      '同じ得意先（不二新製作所など）でも、依頼書で来る場合と図面で来る場合の両方がある。',
      '',
      '技術図面の読み方（表題欄・投影図・寸法がある場合）:',
      '・number は表題欄の図番（図面番号/DWG.NO）をそのまま写す。',
      '・name は表題欄の図面名称（品名/TITLE）をそのまま写す。加工内容から作った名前に置き換えない。',
      '・material は表題欄または注記の材質をそのまま写す。',
      '・size は、ホーニングする穴があれば「ホーニングする穴の径-ワーク全長」（例 ホーニング穴φ30・全長1000 → 30-1000）。',
      '　ホーニングする穴は、ホーニング指示・厳しい公差（H7等や+0.0x）・細かい面粗さが付いた内径のこと。ボルト穴・給油穴・逃がし等の他の穴と取り違えない。',
      '　全長はワーク全体の長さで、穴の深さや部分寸法ではない。ホーニングする穴が判断できない場合だけ、図面の主要寸法表記にする。',
      '',
      ...
      '　この場合 size は仕上げ内径と全長をとって「37-777」、tolerance は「+0.3/+0.1」、素材寸法と下穴は note に残す。',
